...
describe('generateFromMeta', () => {
  ...
  describe('successful generation', () => {
    ...
    it('should handle complex meta structures', async () => {
      ...
      const webContent = JSON.parse(webResult!.content);
      ...
      expect(androidResult!.content).toContain('complex_android');
      expect(androidResult!.content).not.toContain('excluded');

      // Check iOS content
      const iosResult = results.find((r) => r.platform === Platform.IOS);
      expect(iosResult).toBeDefined();
      const iosContent = JSON.parse(iosResult!.content);
